...
public final class NTracker {
     ...
     * 固定的配置字段
     */
    private static Map<String, Object> configFields = NLog.buildMap("postUrl=",
            null, // 上报路径
            "protocolParameter=", null, // 字段缩写字典
            "syncSave=", null, // 是否同步处理
            "passiveSend=", null // 不会主动发送
            );
        ...
        while (iterator.hasNext()) {
            ...
            if ("protocolParameter".equals(key)) {
                if (!(value instanceof Map))
                    continue;
                value = NLog
                        .mergeMap(configFields, (Map<String, Object>) value);
            }
     ...
     * 
     * @param hitType
     *            发送类型，appview、event、timing、exception
     * @param map
     *            参数集合
     */
    public void send(String hitType, Map<String, Object> map) {
        ...
        Map<String, Object> data = NLog.mergeMap(
                NLog.buildMap("sid=", NLog.getSessionId(), // 会话id
        ...
        if (NLog.getBoolean("debug")) {
            Log.v(LOG_TAG, String.format("%s.send() data=%s name=%s fields=%s",
     ...
     *            是否导致崩溃
     */
    public void sendException(String threadName, String description,
            Boolean fatal) {

        send("exception", NLog.buildMap("exThread=", threadName,
                "exDescription=", description, "exFatal=", fatal));
    }
    ...
    public void sendTiming(String category, String var, Long value, String label) {
        ...
        send("timing", NLog.buildMap("timingCategory=", category, "timingVar=",
                var, "timingValue=", value, "timingLabel=", label));
    }
